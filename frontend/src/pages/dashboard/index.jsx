...
export default function Dashboard() {
  ...
  const kpis = [
    { label: "Total Products", value: stats.products, icon: ShoppingBag, color: "text-[var(--color-border)] bg-[var(--color-border)]/10" },
    { label: "Total Orders", value: stats.orders, icon: ListOrdered, color: "text-[#556B2F] bg-[#556B2F]/10" },
    { label: "Total Customers", value: stats.customers, icon: Users, color: "text-[var(--color-primary)] bg-primary/10" },
  ...
  return (
    <div className="space-y-8">
      {/* Welcome header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-bg-secondary p-6 rounded-2xl border border-border/10 shadow-sm">
        <div>
          <h2 className="font-display text-2xl font-light uppercase tracking-wider text-[var(--color-primary)]">
            Overview Dashboard
          </h2>
        ...
        <div className="flex gap-3">
          ...
          <Link
            to="/admin/orders"
            className="inline-flex items-center gap-2 rounded-xl border border-border/20 hover:border-[var(--color-primary)] px-4 py-2.5 text-xs font-semibold uppercase tracking-wider text-[var(--color-primary)] hover:bg-bg-dark/40 transition-colors"
          >
            Manage Orders
          </Link>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {kpis.map((kpi, idx) => (
          <div
            key={idx}
            className="bg-bg-secondary p-6 rounded-2xl border border-border/10 shadow-sm flex items-center justify-between"
          >
            <div>
              <p className="text-[10px] uppercase tracking-wider text-[#6D6D6D] font-semibold">
                {kpi.label}
              </p>
              <h3 className="text-2xl font-bold text-[var(--color-primary)] mt-1">{kpi.value}</h3>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        {/* Recent Orders Table */}
        <div className="lg:col-span-8 bg-bg-secondary rounded-2xl border border-border/10 p-6 shadow-sm flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between pb-4 border-b border-white/10 mb-4">
              <h3 className="font-display text-sm font-semibold uppercase tracking-wider text-[var(--color-primary)]">
                Recent Orders
              </h3>
              <Link
                to="/admin/orders"
                className="text-[10px] font-bold uppercase tracking-wider text-[var(--color-border)] hover:underline flex items-center gap-1"
              >
                View all orders
                <ArrowRight size={12} />
              </Link>
                  ...
                  <tbody>
                    {recentOrders.map((o) => (
                      <tr key={o.id} className="border-b border-white/10 last:border-0 hover:bg-bg-dark/20">
                        ...
                        <td className="py-3 font-semibold text-text-light">₹{o.total}</td>
                        <td className="py-3">
        ...
        <div className="lg:col-span-4 bg-bg-secondary rounded-2xl border border-border/10 p-6 shadow-sm">
          <h3 className="font-display text-sm font-semibold uppercase tracking-wider text-[var(--color-primary)] pb-4 border-b border-white/10 mb-4 flex items-center gap-2">
            <AlertCircle size={16} className="text-[var(--color-border)]" />
            Low Stock Alert
          </h3>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-white/10">
              <div>
                <h4 className="text-xs font-semibold text-text-light">THUKIL Trench Coat</h4>
            ...
            <div className="flex items-center justify-between pb-3 border-b border-white/10">
              <div>
                <h4 className="text-xs font-semibold text-text-light">Tailored Pleated Trousers</h4>
            ...
            <div className="flex items-center justify-between">
              <div>
                <h4 className="text-xs font-semibold text-text-light">Black Satin Slip Dress</h4>
